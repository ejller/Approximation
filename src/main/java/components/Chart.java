...
public class Chart extends JPanel {
    ...
    public Chart(int width, int height, String formula1, String formula2, int dropId, Double[][] points) {
        this.dropId = dropId;
        this.formula1 = new ExpressionBuilder(formula1).variable("x").build();
        this.formula2 = new ExpressionBuilder(formula2).variable("x").build();
    ...
    protected void paintComponent(@Nullable Graphics graphics) {
        ...
        for (int var3 = this.points.length; i < var3; ++i) {
        ...
        double prevY2 = -this.formula2.setVariable("x", prevX).evaluate();
        ...
        for(int x = fromX; x<toX; x+=stepX)  {
            ...
            try {
                ...
                ty2 = -this.formula2.setVariable("x", tx).evaluate();
            } catch (Exception var18) {
                ty1 = -this.formula1.setVariable("x", tx + 1.0E-5D).evaluate();
                ty2 = -this.formula2.setVariable("x", tx + 1.0E-5D).evaluate();
            }
            ...
            graphics.drawLine((int) (prevX * this.coefficientX), (int) (prevY2 * this.coefficientY), (int) (tx * this.coefficientX), (int) (ty2 * this.coefficientY));
            prevX = tx;
            prevY1 = ty1;
            prevY2 = ty2;
        }
